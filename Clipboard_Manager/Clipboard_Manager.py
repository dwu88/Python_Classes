...
class ClipboardManagerApp:
    def __init__(self):
        ...
        self.system_font.configure(family="San Francisco", size=12)
    ...
    def setup_ui(self):
        ...
        self.label = Label(self.root, text="Clipboard History:", font=self.system_font)
        self.label.pack(pady=10)

        # Search bar
        self.search_var = StringVar()
        self.search_var.trace("w", self.filter_history)
        self.search_entry = Entry(self.root, textvariable=self.search_var, font=self.system_font)
        self.search_entry.pack(pady=10, fill="x", padx=10)

        # Create a frame to hold the Listbox and Scrollbar
        listbox_frame = Label(self.root)
        listbox_frame.pack(padx=10, pady=10, fill=BOTH, expand=True)

        # Listbox to display clipboard history
        self.listbox = Listbox(listbox_frame, width=50, height=10, font=self.system_font)
        ...
        self.copied_item_label = Label(self.root, text="Current Copied Item: None", font=self.system_font)
        self.copied_item_label.pack(pady=10)

        # Button to clear clipboard history
        self.clear_button = Button(self.root, text="Clear History", command=self.clear_history, font=self.system_font, bd=0, highlightthickness=0)
        self.clear_button.pack(pady=10)

        # Button to remove selected item
        self.remove_button = Button(self.root, text="Remove Selected", command=self.remove_selected, font=self.system_font, bd=0, highlightthickness=0)
        self.remove_button.pack(pady=10)

        # Button to undo last removal
        self.undo_button = Button(self.root, text="Undo", command=self.undo_last_removal, font=self.system_font, bd=0, highlightthickness=0)
        ...
        self.always_on_top_checkbutton = Checkbutton(
            self.root, text="Always on Top", variable=self.always_on_top_var, command=self.toggle_always_on_top, font=self.system_font
        )
        ...
        # Tip for users
        self.tip_label = Label(self.root, text="Tip: Double-click an item or press Enter to copy it.", font=self.system_font)
    ...
    def highlight_copied_item(self, index):
        ...
        if self.highlighted_item is not None:
            self.listbox.itemconfig(self.highlighted_item, bg=self.listbox.cget("bg"))  # Reset previous highlight
        self.listbox.itemconfig(index, bg="lightgreen")  # Highlight the new item
    ...
    def clear_history(self):
        ...
        if self.highlighted_item is not None:
            self.listbox.itemconfig(self.highlighted_item, bg=self.listbox.cget("bg"))  # Reset highlight
            self.highlighted_item = None
